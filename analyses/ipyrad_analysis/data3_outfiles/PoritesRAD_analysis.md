...
```
##           2         3         1
## 2        NA 0.1942800 0.2069206
## 3 0.1942800        NA 0.1907835
## 1 0.2069206 0.1907835        NA
```
...
## Total response variance: 111.4157 
## Analysis based on 24 observations 
## 
## 10 Regressors: 
## Some regressors combined in groups: 
##         Group  as.factor(sinfo$habitat) : `as.factor(sinfo$habitat)`IRS `as.factor(sinfo$habitat)`LAG `as.factor(sinfo$habitat)`LSG `as.factor(sinfo$habitat)`MGR `as.factor(sinfo$habitat)`OR `as.factor(sinfo$habitat)`TRT `as.factor(sinfo$habitat)`USG 
## 
...
##  
## Proportion of variance explained by model: 59.78%
## Metrics are normalized to sum to 100% (rela=TRUE). 
## 
## Relative importance metrics: 
## 
##                                  lmg        last        first
## as.factor(sinfo$habitat) 0.360032801 0.248088828 0.3979916890
## as.numeric(sinfo$depth)  0.006331622 0.016010070 0.0006968082
## as.factor(sinfo$sym)     0.028222869 0.006960101 0.0397377002
## as.numeric(sinfo$diam)   0.605412708 0.728941001 0.5615738027
## 
## Average coefficients for different model sizes: 
## 
##                                    1group    2groups     3groups
## as.numeric(sinfo$depth)       -0.04106976  0.3612993   0.7207189
## as.factor(sinfo$sym)          -5.26379943 -4.4391303  -1.8675292
## `as.factor(sinfo$habitat)`IRS 10.11381996  2.5221526  -6.1484200
## `as.factor(sinfo$habitat)`LAG  4.57369678  2.3228383   0.4543486
## `as.factor(sinfo$habitat)`LSG  0.29692459 -3.0775287  -7.2443995
## `as.factor(sinfo$habitat)`MGR 14.95559715 10.0751922   4.5095376
## `as.factor(sinfo$habitat)`OR   5.06502902 -1.1586271  -8.4478465
## `as.factor(sinfo$habitat)`TRT 16.44519348 11.8428627   8.6374582
## `as.factor(sinfo$habitat)`USG -4.10793568 -7.9544324 -12.6019046
## as.numeric(sinfo$diam)        -1.51594178 -1.4863554  -1.4820249
##                                   4groups
## as.numeric(sinfo$depth)         0.8209180
## as.factor(sinfo$sym)            2.2019166
## `as.factor(sinfo$habitat)`IRS -12.0090397
## `as.factor(sinfo$habitat)`LAG  -0.2276446
## `as.factor(sinfo$habitat)`LSG -10.1700291
## `as.factor(sinfo$habitat)`MGR  -1.7712753
## `as.factor(sinfo$habitat)`OR  -12.4797810
## `as.factor(sinfo$habitat)`TRT   6.9452793
## `as.factor(sinfo$habitat)`USG -16.1149857
## as.numeric(sinfo$diam)         -1.4947708
```
...
## 
## Call:
## lm(formula = snpdist5[, 3] ~ methdist5[, 3])
## 
## Residuals:
##       Min        1Q    Median        3Q       Max 
## -0.197169 -0.023920 -0.002375  0.027497  0.109394 
## 
## Coefficients:
##                Estimate Std. Error t value Pr(>|t|)    
## (Intercept)     0.15182    0.01699   8.938  < 2e-16 ***
## methdist5[, 3]  0.32117    0.07290   4.406 1.47e-05 ***
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
## 
## Residual standard error: 0.04157 on 298 degrees of freedom
## Multiple R-squared:  0.06115,	Adjusted R-squared:  0.058 
## F-statistic: 19.41 on 1 and 298 DF,  p-value: 1.473e-05
```
...
## 
## Call:
## lm(formula = snpdist6[, 3] ~ methdist6[, 3])
## 
## Residuals:
##       Min        1Q    Median        3Q       Max 
## -0.081780 -0.023342 -0.004919  0.026076  0.110650 
## 
## Coefficients:
##                Estimate Std. Error t value Pr(>|t|)    
## (Intercept)     0.17822    0.01610  11.071  < 2e-16 ***
## methdist6[, 3]  0.21265    0.06894   3.084  0.00223 ** 
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
## 
## Residual standard error: 0.03837 on 296 degrees of freedom
## Multiple R-squared:  0.03114,	Adjusted R-squared:  0.02786 
## F-statistic: 9.513 on 1 and 296 DF,  p-value: 0.002233
```
